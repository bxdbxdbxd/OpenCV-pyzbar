...
class TopZoneCropApp:
    def __init__(self, root):
        ...
        tk.Label(self.frame_images, text="Верхняя зона", font=("Arial", 9, "bold")).grid(row=0, column=1, padx=2,
                                                                                         pady=2)
    ...
    def get_rectangle_corners(self, box, offset_y=0):
        ...
        top_left = (x, global_y)
        top_right = (x + w, global_y)
        bottom_right = (x + w, global_y + h)
        bottom_left = (x, global_y + h)
    ...
    def filter_and_link_pairs(self, items, img_w, base_img):
        ...
        for item in items:
            ...
            for t in tokens:
                if t in {"YELLOW", "MAGENTA", "CYAN", "BLACK", "PANTONE", "ЛАК", "VARNISH", "C", "M", "Y", "K", "К",
                         "С", "М"}:
    ...
    def detect_and_extract_figures(self, img_25, min_fig_area=100, merge_dist=15):
        ...
        main_boxes = [cv.boundingRect(cnt) for cnt in contours if
                      cv.boundingRect(cnt)[2] * cv.boundingRect(cnt)[3] >= min_fig_area]
        ...
        print("\n--- Результаты поиска (координаты углов относительно целого изображения) ---")
        for idx, pair in enumerate(valid_pairs, start=1):
            cmyk_corners = self.get_rectangle_corners(pair['cmyk_box'], self.current_offset_y)
            design_corners = self.get_rectangle_corners(pair['design_box'], self.current_offset_y)

            print(f"\nДизайн №{pair['design_num']} (Цвета: {pair['colors']}):")
            print(f"  [CMYK блок] 4 точки (ЛВ, ПВ, ПН, ЛН): {cmyk_corners}")
            print(f"  [Дизайн блок] 4 точки (ЛВ, ПВ, ПН, ЛН): {design_corners}")
    ...
    def render_figures_gallery(self):
        ...
        for idx, pair in enumerate(self.figures_top25, start=1):
            ...
            tk.Label(f_design, text="Номер дизайна", font=("Arial", 7, "bold"), bg="#F9F9F9", fg="#555").pack(
                side=tk.TOP)
    ...
    def process_and_update(self, status_prefix=""):
        ...
        if valid_zones:
            ...
            if self.img_remainder is not None and self.img_remainder.size > 0:
                ...
                trim_offset_y = 0
                if rem_coords.size > 0:
                    trim_offset_y = rem_coords.min(axis=0)[0]

                self.current_offset_y = ty + th + trim_offset_y
